...
% Big math delimiters live in the LM math-extension font, whose glyphs carry no ToUnicode
% mapping; without these entries \left(...\right) and \left[...\right] extract as control
% characters and as the letters h and i respectively.
\pdfglyphtounicode{parenleftbig}{0028}      \pdfglyphtounicode{parenrightbig}{0029}
\pdfglyphtounicode{parenleftBig}{0028}      \pdfglyphtounicode{parenrightBig}{0029}
\pdfglyphtounicode{parenleftbigg}{0028}     \pdfglyphtounicode{parenrightbigg}{0029}
\pdfglyphtounicode{parenleftBigg}{0028}     \pdfglyphtounicode{parenrightBigg}{0029}
\pdfglyphtounicode{bracketleftbig}{005B}    \pdfglyphtounicode{bracketrightbig}{005D}
\pdfglyphtounicode{bracketleftBig}{005B}    \pdfglyphtounicode{bracketrightBig}{005D}
\pdfglyphtounicode{bracketleftbigg}{005B}   \pdfglyphtounicode{bracketrightbigg}{005D}
\pdfglyphtounicode{bracketleftBigg}{005B}   \pdfglyphtounicode{bracketrightBigg}{005D}
\pdfglyphtounicode{braceleftbig}{007B}      \pdfglyphtounicode{bracerightbig}{007D}
\pdfglyphtounicode{braceleftBig}{007B}      \pdfglyphtounicode{bracerightBig}{007D}
...
\hypersetup{
  ...
  pdfkeywords={Heisenberg representation, Fourier filtration, continuum limit, Mosco convergence,
...
\begin{document}
  ...
  \begin{abstract}
    On a supplied Heisenberg carrier, the admissible fibre carries the
    Heisenberg--Schrödinger representation of $\Heis(\Z/q\Z)$ at a fixed non-trivial central
    character --- not the associated Weil action of the symplectic automorphism group, which is a
    further representation and is not used here --- and the companion
    Foundation paper defines the
    ...
    Third, a normalisation no-go: if the saturation depth diverges and the modulation and translation
    weights both remain positive and of order one --- as the pipeline data indicate --- then no common
    scalar normalisation of the form
    ...
    Whether a spatial continuum can emerge from this structure is exactly the open content of Q5.
  \end{abstract}

  \paragraph{Keywords.}
    Heisenberg representation; Fourier filtration; continuum limit; Mosco convergence; no-go result;
  ...
  \subsection{The question}
    ...
    The admissible fibre $F_n$ carries the Heisenberg--Schrödinger representation $\pi_c$
    of $\Gq = \Heis(\Z/q\Z)$ at a fixed non-trivial central character, on a \emph{supplied} Heisenberg
    carrier $\mathcal{H}_c = L^2(\Z/q\Z)$.
    This is the representation displayed in Section~\ref{sec:setup} and used throughout.
    It is \emph{not} the associated Weil representation: that is the action of the symplectic
    automorphism group of the same carrier, a further representation requiring the symplectic action
    as an additional input~\cite{Beau2026m,Beau2026HeisenbergStructure}.
    Companion papers write $V_\rho$ for the admissible fibre's module; that symbol is not used
    here, and no result below invokes a Weil module.
    That carrier is not selected by the admissibility axioms: they force irreducibility and
    non-commutation, but neither make the commutator central nor single out a finite Heisenberg
    group, an explicit $\mathfrak{S}_3$ countermodel satisfying the extracted carrier
    contract~\cite{Beau2026m,Beau2026HeisenbergStructure}.
    The present paper takes the carrier as given, and indeed fixes it explicitly in
    Section~\ref{sec:setup}, so none of its results depends on how it is obtained.
    The Foundation paper~\cite{Beau2026m} defines, at its
  ...
  \subsection{Results and their statuses}
    ...
    \begin{description}
      ...
      \item[Normalisation no-go (Theorem~\ref{thm:nogo}; proved).]
      Under Hypothesis~\textup{[H-depth]} and the measured weight behaviour:
      if the depth diverges and the modulation and translation weights have positive finite limits,
      then for every common
      ...
      Under the blow-up of the frequency variable, $u = b/n_1(q)$, the rescaled form converges to
      a Dirichlet quadratic form on $L^2([-1,1])$ in the rescaled \emph{frequency} variable, with
      a potential term controlled by the critical coverage $x_1(q)$.
  ...
  \subsection{Group, sector, and filtration}
    ...
    Group elements are written in the coordinates $g = (a, b, \gamma)$ with
    $g = X^{b} Y^{a} Z^{\gamma}$ in normal form, so that $X$ increments the $b$-coordinate and $Y$
    the $a$-coordinate.
    (The O-series pipeline code uses the opposite naming for the two non-central generators; the
    weight limits $A_X, A_Y$ below always refer to the modulation and translation sectors as defined
    here, and Hypothesis~[H-$w'$] is insensitive to the naming since both sectors are covered.)
    ...
    The Heisenberg--Schrödinger representation $\pi_c$ at central character $e^{2\pi i/q}$
    (the corpus convention, $c = 1$) is written $\rho := \pi_c$ throughout, and acts
    by
    \[
    ...
    \]
    The conjugate sector $c = q-1$ satisfies $\rho_{q-1} = \overline{\rho_1}$
    (the conjugation identity, O17; recalled in O18 Theorem~3.1)~\cite{Beau2026a21,Beau2026a22}
    and is related to the $c = 1$ sector anti-linearly; it is never mixed linearly with it.
  ...
  \subsection{Published depth and weights, and their statuses}
    ...
    reduction $n_1(q) = (x_1(q)/\CHeis)^{1/4}\sqrt{q}\,(1 + o(1))$ to the critical coverage
  ...
  \label{sec:filtration}
    ...
    \begin{proof}
      ...
      $\beta(B_n) \subseteq \{-n, \ldots, n\}$; conversely $X^{b} \in B_n$ for $|b| \le n$, so equality
      holds.
    ...
    \begin{remark}[Physical bandwidth of the canonical stage]
      ...
      $n_1(q)\,h = \sqrt{2\pi}\,(x_1(q)/\CHeis)^{1/4}\,(1 + o(1))$, by the reduction
      of~\cite{Beau2026ccnote}.
      The bandwidth is therefore governed by the critical coverage: it is bounded away from zero if and
      only if $x_1(q)$ is.
    \end{remark}

    \begin{corollary}[Mode capture]
      \label{cor:capture}
      A fixed toric mode $e_m$ lies in $\Omega_{n_1(q)}$ if and only if
      $n_1(q) \ge |m|$; under Hypothesis~\textup{[H-depth]}, every fixed toric mode is captured
      permanently.
    \end{corollary}

    \paragraph{Numerical observation (balanced rejection; empirical, measured primes).}
    At the measured primes, balanced (line-scale) Hermite profiles are rejected: their relative
    distance to the stage is non-monotone in $q$ with a rising tail
    (for the Gaussian profile, $0.051,\, 0.010,\, 0.007,\, 0.010,\, 0.020$ over
    $q = 29, 61, 101, 151, 211$; for the sixth Hermite profile, non-monotone within
    $0.66$--$0.75$ with endpoints $0.75 \to 0.74$), tracking the non-monotone physical bandwidth of
    Remark~\ref{rem:bandwidth}.
    This is a reproducible numerical statement (script \texttt{code/canonical\_sector\_test.py},
    deterministic, published depths only), not a theorem.
  ...
  \label{sec:zero}
    ...
    \begin{theorem}[Zero-form limit]
      ...
      Consequently $\Eq \to 0$ uniformly on norm-bounded sets, and the Mosco
      limit~\cite{KuwaeShioya2003} of the published forms, along any isometric identification of the
      stages with subspaces of a fixed Hilbert space whose ranges have asymptotically dense union, is
      the zero form.
    \end{theorem}
  ...
  \label{sec:nogo}
    ...
    > 0$ for every non-zero $f$, and the $Y$-sum equal to
    ...
    \begin{theorem}[No common scalar normalisation yields a toric differential limit]
      ...
      Then every subsequential limit of the rescaled forms $\alpha_q\,\mathcal{B}_q$ on toric targets
      with finitely many Fourier modes is one of:
      \begin{enumerate}
        ...
        \item[\textup{(iii)}] the degenerate form equal to $+\infty$ on every non-zero target (when
        $\alpha_q \to \infty$).
      \end{enumerate}
    ...
    \begin{proof}
      ...
      $q^{-2}$);
      $\to +\infty$ if $\alpha_q \to \infty$, since $c_X(f) > 0$ for every non-zero $f$.
  ...
  \label{sec:blowup}
    ...
    \begin{proposition}[Dual blow-up limit]
      ...
      \]
      along any subsequence on which $x_1(q)/\CHeis \to \lambda \in [0, \infty)$, using the
      ...
      would be an oscillator-type Schrödinger form in the dual variable.
    \end{proposition}
  ...
  \section{Heisenberg--Schrödinger representation: explicit generator actions}
  \label{app:weil}
  ...
  representation at this character.
  It is a conditional endpoint: it fixes the carrier once a finite Heisenberg group and a non-trivial
  central character are supplied, and does not itself select
  either~\cite{Beau2026m,Beau2026HeisenbergStructure}.
